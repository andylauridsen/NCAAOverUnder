...
def calculate_live_over_under(score_a, score_b, time_left, team_a_fouls, team_b_fouls, bonus_a, bonus_b, timeouts_a, timeouts_b, seed_a, seed_b, round_num):
    ...
    estimated_remaining_points = (current_total / (40 - time_left)) * time_left

    foul_factor = 1 + (0.05 if bonus_a or bonus_b else 0)
    ...
    if timeouts_a + timeouts_b > 3:
        estimated_remaining_points *= 0.95
...
st.markdown("---")
st.markdown("**Version 1.4 – Fixed full-game time input bug and updated versioning**")
